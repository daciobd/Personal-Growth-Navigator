...
type ChallengeState =
  | { type: "idle" }
  | { type: "loading" }
  | { type: "challenge"; aiAction: string; practice: Practice; practiceIndex: number }
  | { type: "checking-in"; aiAction: string; practice: Practice; practiceIndex: number }
  | { type: "submitting" }
...
function XPPop({ xp }: { xp: number }) {
  const colors = Colors.light;
  return (
    <Animated.View entering={FadeInDown.springify()} style={[styles.xpPop, { backgroundColor: colors.accent }]}>
      <Feather name="zap" size={14} color="#fff" />
      <Text style={[styles.xpPopText, { fontFamily: "Inter_700Bold" }]}>+{xp} XP</Text>
    </Animated.View>
  );
}
...
export function DailyChallenge() {
  ...
  if (state.type === "no-plan") return null;

  if (state.type === "challenge" || state.type === "checking-in") {
    const aColor = APPROACH_COLORS[state.practice.abordagem] ?? { bg: "#F5F5F5", text: "#333" };
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          <View style={styles.cardHeader}>
            <View style={[styles.challengeBadge, { backgroundColor: colors.primary }]}>
              ...
              <Text style={[styles.challengeBadgeText, { fontFamily: "Inter_600SemiBold" }]}>
                Desafio de hoje
              </Text>
            </View>
            <View style={[styles.approachBadge, { backgroundColor: aColor.bg }]}>
              <Text style={[styles.approachText, { color: aColor.text, fontFamily: "Inter_500Medium" }]}>
          ...
          {isCheckingIn && (
            <Animated.View entering={FadeInDown.duration(300)} style={styles.checkinForm}>
              <Text style={[styles.checkinLabel, { color: colors.text, fontFamily: "Inter_600SemiBold" }]}>
                Conseguiu fazer?
              </Text>
              <View style={styles.yesNoRow}>
                <Pressable
                  ...
                  style={[
                    styles.yesNoBtn,
                    {
                      backgroundColor: completed === true ? colors.success : colors.background,
                      borderColor: completed === true ? colors.success : colors.cardBorder,
                    },
                  ]}
                >
                  <Feather name="check" size={16} color={completed === true ? "#fff" : colors.textMuted} />
                  <Text
                    style={[
                      styles.yesNoText,
                      {
                ...
                <Pressable
                  ...
                  style={[
                    styles.yesNoBtn,
                    {
                      backgroundColor: completed === false ? colors.danger : colors.background,
                      borderColor: completed === false ? colors.danger : colors.cardBorder,
                    },
                  ]}
                >
                  <Feather name="x" size={16} color={completed === false ? "#fff" : colors.textMuted} />
                  <Text
                    style={[
                      styles.yesNoText,
                      {
              ...
              <Pressable
                ...
                style={[
                  styles.btn,
                  {
                    backgroundColor: completed !== null ? colors.primary : colors.cardBorder,
                  },
                ]}
              >
                <Text style={[styles.btnText, { fontFamily: "Inter_600SemiBold" }]}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
          <View style={styles.doneHeader}>
            ...
            <View style={styles.doneTexts}>
              ...
              <Text style={[styles.doneStreak, { color: colors.textSecondary, fontFamily: "Inter_400Regular" }]}>
                {state.streak} dia{state.streak !== 1 ? "s" : ""} seguido{state.streak !== 1 ? "s" : ""}
              </Text>
            </View>
          ...
          <View style={[styles.tipBox, { backgroundColor: colors.background }]}>
            ...
            <Text style={[styles.tipText, { color: colors.text, fontFamily: "Inter_400Regular" }]}>
              {state.aiTip}
            </Text>
          </View>
        </View>
